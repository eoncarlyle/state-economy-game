...
const data = {
  ...
  children: [
    {
      gdpCategory: "Private industries",
      gdp: 98585.1,
      children: [
        {
    ...
    {
      gdpCategory: "Government and government enterprises",
      gdp: 12286.0,
      children: [
...
leaf
  .append("rect") //.attr("id", (d) => (d.leafUid = DOM.uid("leaf")).id)
  .attr("fill", (d) => {
...
leaf
  .append("clipPath") //.attr("id", (d) => (d.clipUid = DOM.uid("clip")).id)
  .append("use");
...
leaf
  ...
  .selectAll("tspan")
  .data((d) => d.data.gdpCategory.split(/(?=[A-Z][a-z])|\s+/g).concat(format(d.gdp)))
  .join("tspan")
  .attr("x", 3)
